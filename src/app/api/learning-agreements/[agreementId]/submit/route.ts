...
import { submitAgreement } from '@/modules/learning-agreements/service';
...
export async function POST(request: NextRequest, { params }: { params: { agreementId: string } }) {
  ...
  try {
    ...
    const agreement = await submitAgreement({ agreementId: params.agreementId, userId: payload.userId });
    ...
    console.error(`[POST /api/learning-agreements/${params.agreementId}/submit]`, body, error);
